...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Get only the latest version for each content
    const contentsWithLatestVersion = contents.map((content: any) => ({
      ...content,
      author: content.creator, // Map creator to author for frontend compatibility
      versions: content.versions.sort((a: any, b: any) => b.version - a.version).slice(0, 1),
    }));

    return NextResponse.json(contentsWithLatestVersion);
  } catch (error) {
